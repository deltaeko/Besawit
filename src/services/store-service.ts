...
export async function createStoreSaleTx(payload: unknown, actorId?: string | null) {
  return runInDbTransaction(async () => {
    ...
    const receivable = await createReceivableEntry({
      ...
      dueDate:
        parsed.saleType === "credit" && parsed.dueDate
          ? new Date(parsed.dueDate)
          : new Date(parsed.transactionDate),
      notes:
        parsed.saleType === "cash"
          ? "Auto-generated from cash store sale"
          : "Auto-generated from store sale",
      createdBy: actorId,
    });

    if (parsed.saleType === "cash") {
      await postPayment(
        {
          ...
          amount: Number(sale.totalAmount),
          notes: "Auto-posted cash payment from store sale",
        },
        actorId,
      );
    }

    const finalSale = await getStoreSaleById(sale.id);
    ...
    await logAudit({
      ...
      action: "create",
      actorId,
      after: finalSale ?? sale,
    });

    return finalSale ?? sale;
  });
}
