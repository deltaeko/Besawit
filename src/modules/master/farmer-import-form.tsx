...
export function FarmerImportForm() {
  ...
  async function requestImport(action: "preview" | "apply") {
    if (!file) {
      toast.error("Pilih file Excel atau CSV petani terlebih dahulu.");
      return;
    }
    ...
    const response = await fetch("/api/farmers/import", {
      method: "POST",
      body: formData,
    });
    ...
    if (!response.ok) {
      toast.error(result?.error ?? "Gagal memproses import petani.");
    ...
    if (action === "preview") {
      ...
      setApplyResult(null);
      toast.success("Preview import petani berhasil dibuat.");
      setLoadingPreview(false);
      return;
    }

    setApplyResult(result as ApplyResult);
    toast.success("Import petani selesai diproses.");
    setApplying(false);
  }
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title="File Import"
        description="Gunakan file Excel hasil unduhan template. Sistem akan membaca data, menampilkan preview, lalu membuat atau memperbarui petani berdasarkan kode."
      >
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="farmer-import-file">File Import Petani</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="duplicateStrategy">Perlakuan Jika Kode Petani Sudah Ada</Label>
              <Select
              ...
              >
                <option value="update_existing">Update petani existing berdasarkan kode</option>
                <option value="skip_existing">Lewati petani existing</option>
              </Select>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button asChild type="button" variant="ghost">
                <Link href="/api/farmers/import/template?format=csv">Unduh CSV</Link>
              </Button>
              <Button
          ...
          <div className="rounded-2xl border border-border/80 bg-muted/20 p-5">
            ...
            <div className="mt-5 rounded-2xl border border-dashed border-border/80 bg-background/70 p-4 text-sm text-muted-foreground">
              Pastikan kode petani unik. Jika kode yang sama sudah ada, sistem akan update atau lewati sesuai strategi yang dipilih.
            </div>
          </div>
        </div>
      </SectionCard>
      ...
      {preview ? (
        <SectionCard
          ...
          description="Periksa hasil validasi sebelum data benar-benar dimasukkan ke master petani."
        >
                  ...
                  <TableBody>
                    {previewRows.map((row) => (
                      <TableRow key={`${row.lineNumber}-${row.code}`}>
                        ...
                        <TableCell>{row.districtOrCity || "-"}</TableCell>
      ...
      {applyResult ? (
        <SectionCard
          title="Hasil Import"
          description="Ringkasan proses import setelah data dijalankan ke master petani."
        >
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <div className="rounded-2xl border bg-muted/20 p-4">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Petani Baru</div>
              <div className="mt-2 text-lg font-semibold">{applyResult.summary.createdRows}</div>
            </div>
            <div className="rounded-2xl border bg-muted/20 p-4">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Petani Diupdate</div>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Button asChild variant="outline">
              <Link href="/master/farmers">Kembali ke Daftar Petani</Link>
            </Button>
            <Button asChild>
              <Link href="/master/farmers/new">Tambah Petani Manual</Link>
            </Button>
          </div>
        </SectionCard>
      ) : null}
